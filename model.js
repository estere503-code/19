...
export const governanceConditions = [
  { name: "Ella Fast ramp control", category: "Part 1 constraint", detail: "May 0%, June 25%, July 50%, August onward 100% productivity. No new revenue or cost is created by this control.", evidence: "06 HR and Personnel Plan · Ella Fast status; 03 Sales Director Plan" },
  { name: "Placeholder programme gate", category: "Governance condition / risk", detail: "Do not add revenue without a dated programme, delivery resource, room/venue, sales owner and direct-cost estimate.", evidence: "05 Operations and Trainer Plan · calendar note; 00 Start Here · Part 2" },
];
...
export const initialState = { part1: [
  ...
  { name: "Remove Anna-owned March–December programme revenue", owner: "HR / Sales", category: "Modelling assumption", source: "06 HR and Personnel Plan · Anna Power resignation 28 February", rationale: "This is a modelling assumption caused by the lack of a documented handover, not proof that the programmes were cancelled. Any recovered pipeline requires a separately documented Part 2 handover decision.", revenueLines: employeeControls.annaRevenueLines, revenueDelta: [0, 0, 0, 0, 0, 0, 0, 0, -15400, 0, -5000, 0], directDelta: [0, 0, 0, 0, 0, 0, 0, 0, 7420, 0, 2500, 0], opexDelta: zero() },
...
export const opexBreakdown = [
  ...
  { label: "Employer payroll", value: 35100 }, { label: "Marketing", value: 52500 }, { label: "Rent & utilities", value: 21600 },
  { label: "Software", value: 5400 }, { label: "Accounting & legal", value: 7200 }, { label: "Insurance", value: 1800 },
  { label: "Admin travel", value: 4200 }, { label: "Depreciation", value: 9600 }, { label: "Interest", value: 3000 }, { label: "Other expenses", value: 4200 },
];
...
export function capacityChecks(events = programRegister) {
  ...
  return MONTHS.map((month) => ({ month, days: days[month], capacity: senior[month], trainerOver: days[month] > senior[month], assistantDays: assistants[month], assistantCapacity: 4, assistantOver: assistants[month] > 4, assistantExtraDays: Math.max(0, assistants[month] - 4), openSeats: openSeats[month], openOver: openSeats[month] > 20 }));
}

export function assistantCapacityChecks(events = programRegister) {
  return capacityChecks(events).map((row) => ({ ...row, status: row.assistantOver ? "Review" : "Within capacity", detail: row.assistantOver ? `${row.assistantDays} assistant-days required; ${row.assistantExtraDays} extra day${row.assistantExtraDays === 1 ? "" : "s"} must be individually booked before ${row.month}.` : "Four assistant-days available under the planning rule." }));
}
